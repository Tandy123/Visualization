...
#include "marchingCubes_defines.h"
...
char *volumeFilename = "head256.raw";
...
main(int argc, char **argv)
{
	...
	std::cout<<"choose a raw file :"<<std::endl;
	std::cout<<"1.Bear.raw"<<std::endl;
	std::cout<<"2.Head.raw"<<std::endl;
	std::cout<<"3.Bonsai.raw"<<std::endl;
	std::cout<<"4.MyVolume"<<std::endl;

	int num;
	std::cin>>num;
	switch(num){
	case 1:
		volumeFilename = "bear.raw";
		gridSizeLog2.x = 7;
		gridSizeLog2.y = 7;
		gridSizeLog2.z = 6;
		break;
	case 2:
		volumeFilename = "head.raw";
		gridSizeLog2.x = 8;
		gridSizeLog2.y = 8;
		gridSizeLog2.z = 8;
		break;
	case 3:
		volumeFilename = "bonsai.raw";
		gridSizeLog2.x = 8;
		gridSizeLog2.y = 8;
		gridSizeLog2.z = 8;
		break;
	case 4:
		...
		std::cin >> a1c_fileName;
		volumeFilename = a1c_fileName;
		std::cout<<"input the grid size:"<<std::endl;
		std::cin>> gridSizeLog2.x>> gridSizeLog2.y>> gridSizeLog2.z;
		break;
	}
...
{
	gridSize = make_uint3(1<<gridSizeLog2.x, 1<<gridSizeLog2.y, 1<<gridSizeLog2.z);//网格尺寸
    ...
    numVoxels = gridSize.x*gridSize.y*gridSize.z;//体素个数
    voxelSize = make_float3(2.0f / gridSize.x, 2.0f / gridSize.y, 2.0f / gridSize.z);//体素尺寸
	...
	file_path.append(volumeFilename);
	char *path = (char *) malloc(file_path.length() + 1);
	STRCPY(path, file_path.length() + 1, file_path.c_str());  
...
/*    glClearColor(0.1f, 0.2f, 0.3f, 1.0f);*/
	glClearColor(0.54f, 0.16f, 0.88f, 1.0f);
